...
let gen_rules sctx t ~dir ~scope =
  ...
  let cinapsed_files =
    File_tree.files_of (Super_context.file_tree sctx)
      (Path.Build.drop_build_context_exn dir)
    |> Path.Source.Set.to_list
    |> List.filter_map ~f:(fun p ->
      if Predicate_lang.exec t.files (Path.Source.basename p)
           ~standard:Predicate_lang.true_ then
  ...
  let preprocess =
    Preprocessing.make sctx
      ~dir
      ~expander
      ~dep_kind:Required
      ~lint:Dune_file.Preprocess_map.no_preprocessing
      ~preprocess:t.preprocess
      ~preprocessor_deps:(Super_context.Deps.interpret sctx
          ...
          (A.run (Ok cinaps_exe) ["-diff-cmd"; "-"] ::
           List.map cinapsed_files ~f:(fun fn ->
             ...
             A.diff ~optional:true
               fn (Path.extend_basename fn ~suffix:".cinaps-corrected")))))
